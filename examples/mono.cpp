    ...
    void processData()
    {
      ...
      while (it!=v.end())
      {
        ...
        if (!fs::is_directory(*it))
        {
          ...
          string lc_id = "";
          bool valid = lc_.getLoopClosure(lc_id);
          ...
          if (valid)
          {
            ...
            if (ground_truth.size() > 0)
            {
              ...
              {
                int img_j = boost::lexical_cast<int>(lc_id) - gt_tolerance_ + i;
                if (img_j<0) img_j = 0;
          ...
          // Log
          ROS_INFO_STREAM( cur_filename << " cl with " << lc_id << ": " << valid << ": " << tp << "|" << fa);
          fstream f_log(log_file_str.c_str(), ios::out | ios::app);
          f_log << cur_filename << "," <<
                   lc_id << "," <<
                   boost::lexical_cast<int>(lc_id) << "," <<
                   valid << "," <<
                   tp << "," <<
                   fa << endl;
          f_log.close();
